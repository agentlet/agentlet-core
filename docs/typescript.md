...
The declarations in `src/types/public-api.d.ts` are hand-written: `public-api.d.ts` ships standalone as `dist/agentlet-core.d.ts`, so it cannot `import` from the rest of `src/` at publish time. Instead, the shared shape types (`ModuleConfig`, `EventBusAPI`, `ThemeManagerAPI`, `ZIndexConstants`, `PanelManagerAPI`, ...) are single-sourced here and the real runtime classes under `src/` (`src/index.ts`, `src/core/*.ts`, `src/ui/*.ts`, and the `src/utils/**` managers) `import type` them from this file, so both sides describe the same shape instead of being hand-maintained independently. `npm run typecheck` checks the result by compiling `tests/types/public-api.test-d.ts`, a type-only test that both uses the API the way an agentlet author would (asserting incorrect usage is rejected with `@ts-expect-error`) and asserts bidirectional assignability between each real class and its declaration here, so drift between them fails the build. That test file is not run by Jest; it exists purely for `tsc` to check.

`src/core/GlobalAPI.ts` has no dedicated conformance check here: it has no single public-facing declared type of its own (it builds up the pieces of `AgentletAPI` described elsewhere in this file), so there is nothing to assert bidirectional assignability against.

`AgentletCore` (`src/index.ts`) is checked in one direction only: the test assigns an `AgentletCore` instance to `AgentletAPI` and to the internal core shapes that `UIManager` and `PanelManager` depend on (`UIManagerCore`, `PanelManagerCore`), so drift between the core and any of those fails `npm run typecheck`.

The only files under `src/` still in plain JavaScript are `src/libraries/LibraryLoader.js` and `src/libraries/LibrarySetup.js`. The part of `LibrarySetup` exposed to agentlets (`LibrarySetupAPI`, `window.agentlet.librarySetup`) is still traced back and hand-maintained directly in `public-api.d.ts`, with no conformance check.
